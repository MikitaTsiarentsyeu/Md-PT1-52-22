...
if answer == 'y':
    ...
    if p<=0:
        print ('Incorrect')
    r = float(input('Enter the deposit interest rate in %\n'))
    if r<=0:
        print ('Incorrect')
    ...
    if a == 'm': 
        c = Decimal(p*((1+r/100/12)**(t*12)))
        print(f'Your month amount is: {c.quantize(Decimal("1.00"), ROUND_HALF_UP)}')
    elif a == 'y':
        c = Decimal(p*(1+(r/100*t)))
        print(f'Your year amount is: {c.quantize(Decimal("1.00"), ROUND_HALF_UP)}')
